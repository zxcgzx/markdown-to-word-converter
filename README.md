# AI 智能 Markdown 转 Word · 融合体验版 v5.1.1

一个完全在浏览器中运行的 Markdown 编辑、实时预览与 Word 导出工具。v5.1.1 保留 v5.1 的导出检查、公式源码定位和独立保存状态，针对顶部操作区做了一次专门的布局修复：**文档级操作与编辑工具拆成上下两层，取消横向滚动和右侧覆盖层，所有可见按钮都拥有稳定的点击区域**。原版密码入口、玻璃态卡片、渐变标题区、用户身份和四套主题继续保留。
...
![v5.1.1 密码入口](docs/screenshots/login-v5.1.1.png)

### 1366 像素桌面端：双层常驻工具栏

![v5.1.1 桌面端双层工具栏](docs/screenshots/toolbar-v5.1.1-1366.png)

### 1024 像素紧凑布局：低频功能进入“更多”

![v5.1.1 紧凑工具栏](docs/screenshots/toolbar-v5.1.1-1024.png)

![v5.1.1 更多菜单](docs/screenshots/toolbar-v5.1.1-1024-more.png)

### 390 像素移动端

![v5.1.1 移动端工作区](docs/screenshots/toolbar-v5.1.1-390.png)

### 公式诊断与源码定位

![公式诊断](docs/screenshots/diagnostics-v5.1.1.png)
...
```text
第一层：文件 · 文档名 · 视图 · 输出
第二层：标题/粗体/斜体等格式 · 表格 · AI 修复 · 清空
```

“下载 Word”仍是唯一主按钮，固定在第一层右侧；“清空”被移到第二层最右端并使用弱危险色，减少误触。

### 2. 取消横向滚动和右侧遮挡

旧布局依赖单行横向滚动，同时输出组使用右侧 `sticky`，在部分宽度下会压住视图按钮，造成看得见却点不到。v5.1.1 改为 CSS Grid 双层布局：

- 工具栏 `overflow: visible`，不再裁切控件；
- 输出组恢复普通网格项，不再覆盖其他区域；
- 文档行和编辑行拥有独立点击空间；
- 1366、1920、1180、1024、800 和 390 像素均无按钮重叠或页面横向溢出。

### 3. 更大的点击目标

格式按钮统一提高到至少 `34 × 36 px`，普通工具按钮至少 36 px 高，并补充清晰的键盘焦点环。鼠标、触控板和键盘操作都更稳定。

### 4. 响应式取舍更明确

- 1180 像素及以下：保留新建、打开、文档名、视图和 Word，低频编辑项进入“更多”；
- 900 像素及以下：进一步隐藏“新建”，保留“打开”作为文件入口；
- 680 像素及以下：只保留打开、三种视图和 Word 下载；
- “更多”菜单关闭时完全退出布局，打开后限制高度并可滚动，不会超出视口。
...
- 双层常驻顶部操作栏：第一层处理文件、文档名、视图和输出，第二层处理编辑工具；
- “下载 Word”是唯一高亮主按钮；
...
从 v5.1 覆盖升级时，会话键和桌面/移动视图存储键保持不变，因此不会仅因工具栏补丁而强制清除现有本地设置。
...
- `tests/toolbar-layout.test.js`：6 项双层工具栏、点击区域、更多菜单和响应式布局测试。
